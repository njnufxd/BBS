...
public class BBSPostServiceImpl implements BBSPostService {
    ...
    public PageResult getBBSPostPageForIndex(PageQueryUtil pageUtil) {
        ...
        System.out.println(bbsPostList);
        ...
        System.out.println(bbsPostListEntities);
    ...
    public PageResult getBBSPostPageForAdmin(PageQueryUtil pageUtil) {
        //查询帖子数据
        int total = bbsPostMapper.getAdminBBSPosts(pageUtil);
        List<BBSPost> bbsPostList = bbsPostMapper.findAdminBBSPostList(pageUtil);
        System.out.println(bbsPostList);
        List<BBSPostListEntity> bbsPostListEntities = new ArrayList<>();
        //数据格式转换
        if (!CollectionUtils.isEmpty(bbsPostList)) {
    ...
    public List getHotTopicBBSPostList() {
        ...
        List<TopicBBSPostListEntity> hotTopicBBSPostList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(bbsPostList)) {
            hotTopicBBSPostList = BeanUtil.copyList(bbsPostList, TopicBBSPostListEntity.class);
        }
        return hotTopicBBSPostList;
    }

    @Override
    public List getNewTopicBBSPostList() {
        List<BBSPost> bbsPostList = bbsPostMapper.getNewTopicBBSPostList();
        List<TopicBBSPostListEntity> newTopicBBSPostList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(bbsPostList)) {
            newTopicBBSPostList = BeanUtil.copyList(bbsPostList, TopicBBSPostListEntity.class);
        }
        return newTopicBBSPostList;
    }
    ...
    public Boolean approvePost(Long postId) {
        return bbsPostMapper.approvePost(postId)>0;
    }

    @Override
    public Boolean banPost(Long postId) {
        return bbsPostMapper.banPost(postId)>0;
    }
}
