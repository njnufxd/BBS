...
public class BBSUserController {
    ...
    public List<BBSUser> getAllUsers() {
        return bbsUserService.getAllUsers();
    }
...
//        }
    ...
    @GetMapping("/adminUserCenter/{userId}")
    public String adminUserCenterPage(HttpServletRequest request, @PathVariable("userId") Long userId) {
        //基本用户信息
        BBSUser bbsUser = bbsUserService.getUserById(userId);
        if (bbsUser == null) {
            return "error/error_404";
    ...
    @ResponseBody
    public Result<BBSUser> approveUser(@RequestParam("userId") Long userId) {
        BBSUser bbsUser = bbsUserService.getUserById(userId);
        if (bbsUser.getUserStatus()==1){
            return new Result<>(200,bbsUser);
        }
        if (bbsUserService.approveUser(userId)){
            byte approveStatus=1;
            bbsUser.setUserStatus(approveStatus);
            return new Result<>(200,bbsUser);
        }
        return new Result<>(400,"更新失败，请刷新页面");
    }

    @PostMapping("/admin/banUser")
    @ResponseBody
    public Result<BBSUser> banUser(@RequestParam("userId") Long userId) {
        //基本用户信息
        BBSUser bbsUser = bbsUserService.getUserById(userId);
        if (bbsUser.getUserStatus() == 2) {
            return new Result<>(200, bbsUser);
        }
        if (bbsUserService.banUser(userId)) {
            byte approveStatus = 2;
            bbsUser.setUserStatus(approveStatus);
            return new Result<>(200, bbsUser);
        }
        return new Result<>(400, "更新失败，请刷新页面");
