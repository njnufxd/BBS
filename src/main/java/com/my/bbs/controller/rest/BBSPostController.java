...
public class BBSPostController {
    ...
    @PostMapping("/admin/approvePost")
    @ResponseBody
    public Result<BBSPost> approvePost(@RequestParam("postId") Long postId) {
        BBSPost bbsPost = bbsPostService.getBBSPostById(postId);
        if (bbsPost.getPostStatus()==1){
            return new Result<>(200,bbsPost);
        }
        if (bbsPostService.approvePost(postId)){
            byte approveStatus=1;
            bbsPost.setPostStatus(approveStatus);
            return new Result<>(200,bbsPost);
        }
        return new Result<>(400,"更新失败，请刷新页面");
    }

    @PostMapping("/admin/banPost")
    @ResponseBody
    public Result<BBSPost> banPost(@RequestParam("postId") Long postId) {
        //基本用户信息
        BBSPost bbsPost = bbsPostService.getBBSPostById(postId);
        if (bbsPost.getPostStatus() == 2) {
            return new Result<>(200, bbsPost);
        }
        if (bbsPostService.banPost(postId)) {
